...
(function () {
  ...
  // Until you do, the app works fine using your browser's localStorage.
  // Get config from: Firebase Console → Project Settings → Your apps → SDK
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  // ── In-memory cache (all reads come from here — fast & sync) ──────────────
  const C = { users:[], tickets:[], customers:[], notifications:[], settings:{}, tSeq:0, cSeq:0 };

  // Ready promise — resolves as soon as data is available (localStorage or Firebase)
  let _resolve;
  const _ready = new Promise(r => { _resolve = r; });

  // Swappable save backend — points to localStorage or Firebase
  let _save = () => {};

  // ── localStorage keys ──────────────────────────────────────────────────────
  const K = { U:'ncm_users', T:'ncm_tickets', C:'ncm_customers', N:'ncm_notifs', S:'ncm_settings', TS:'ncm_tseq', CS:'ncm_cseq' };
  ...
  // ── localStorage backend initialiser ──────────────────────────────────────
  function _initLS() {
    if (load(K.U, null) === null) {
      // First run — write seed data
      const s = _getSeed();
      C.users=s.users; C.tickets=s.tickets; C.tSeq=s.tSeq;
      C.customers=s.customers; C.cSeq=s.cSeq;
      C.notifications=s.notifications; C.settings=s.settings;
    } else {
      C.users         = load(K.U,  []);
    ...
    _save = _lsSave;
    // Persist whatever is now in cache
    ...
    _save('settings',      C.settings);
    _resolve();
    console.log('[NexCRM] Using localStorage (data is local to this browser).');
  }

  // ── Firebase backend initialiser ───────────────────────────────────────────
  async function _initFirebase() {
    if (!firebase.apps.length) firebase.initializeApp(FIREBASE_CONFIG);
    const db  = firebase.firestore();
    const COL = db.collection('nexcrm');

    _save = (doc, data) => COL.doc(doc).set(data).catch(e => console.warn('[Firestore] write error:', doc, e));

    const check = await COL.doc('users').get();

    if (!check.exists) {
      // First run on this Firebase project — seed it
      const s = _getSeed();
      C.users=s.users; C.tickets=s.tickets; C.tSeq=s.tSeq;
      C.customers=s.customers; C.cSeq=s.cSeq;
      C.notifications=s.notifications; C.settings=s.settings;
      ...
      const [u,t,cu,n,se] = await Promise.all(
        ['users','tickets','customers','notifications','settings'].map(d => COL.doc(d).get())
      );
    ...
    _resolve();
    console.log('[NexCRM] Connected to Firebase — changes sync across all browsers.');

    // Real-time listeners (run after resolve so they don't trigger on initial load)
    COL.doc('tickets').onSnapshot(d => {
      if (!d.exists) return;
      C.tickets = d.data().items||[]; C.tSeq = d.data().seq||0;
      NexCRM._onDataUpdate?.();
    });
    COL.doc('customers').onSnapshot(d => {
      if (!d.exists) return;
      C.customers = d.data().items||[]; C.cSeq = d.data().seq||0;
      NexCRM._onDataUpdate?.();
    });
    COL.doc('users').onSnapshot(d => {
      if (!d.exists) return;
      C.users = d.data().items||[];
      NexCRM._onDataUpdate?.();
    });
    COL.doc('notifications').onSnapshot(d => {
      if (!d.exists) return;
      C.notifications = d.data().items||[];
      NexCRM._onDataUpdate?.();
    });
  }

  // ── CRUD modules — identical API regardless of backend ────────────────────
  const Users = {
    ...
    create(data) {
      ...
      C.users.push(u); _save('users',{ items:C.users }); return u;
    },
    update(id, data) {
      const i=C.users.findIndex(u=>u.id===id); if(i<0)return null;
      C.users[i]={...C.users[i],...data}; _save('users',{items:C.users}); return C.users[i];
    },
  ...
  const Tickets = {
    ...
    create(data) {
      ...
      C.tickets.push(t); _save('tickets',{items:C.tickets,seq:C.tSeq}); return t;
    },
    update(id, data) {
      const i=C.tickets.findIndex(t=>t.id===id||t.number===id); if(i<0)return null;
      C.tickets[i]={...C.tickets[i],...data,updatedAt:now()}; _save('tickets',{items:C.tickets,seq:C.tSeq}); return C.tickets[i];
    },
    addComment(id, comment) {
      const i=C.tickets.findIndex(t=>t.id===id||t.number===id); if(i<0)return null;
      const c={...comment,id:uid(),createdAt:now()};
      (C.tickets[i].comments=C.tickets[i].comments||[]).push(c);
      C.tickets[i].updatedAt=now(); _save('tickets',{items:C.tickets,seq:C.tSeq}); return c;
    },
  ...
  const Customers = {
    ...
    create(data) {
      ...
      C.customers.push(c); _save('customers',{items:C.customers,seq:C.cSeq}); return c;
    },
    update(id, data) {
      const i=C.customers.findIndex(c=>c.id===id); if(i<0)return null;
      C.customers[i]={...C.customers[i],...data}; _save('customers',{items:C.customers,seq:C.cSeq}); return C.customers[i];
    },
  ...
  const Notifications = {
    ...
    add(n) {
      ...
      _save('notifications',{items:C.notifications}); return notif;
    },
  ...
  // ── Choose backend ─────────────────────────────────────────────────────────
  const FIREBASE_CONFIGURED = !FIREBASE_CONFIG.apiKey.startsWith('PASTE_');

  if (!FIREBASE_CONFIGURED) {
    // Config not set yet — use localStorage immediately, no waiting
    _initLS();
  } else {
    // Try Firebase; if it fails or times out → fall back to localStorage
    let resolved = false;

    const fallbackTimer = setTimeout(() => {
      if (!resolved) {
        console.warn('[NexCRM] Firebase timed out — using localStorage.');
        resolved = true;
        _initLS();
      }
    }, 7000);
    ...
    _initFirebase()
      .then(() => { resolved = true; clearTimeout(fallbackTimer); })
      .catch(err => {
        if (!resolved) {
          console.warn('[NexCRM] Firebase error — using localStorage:', err.message);
          resolved = true;
