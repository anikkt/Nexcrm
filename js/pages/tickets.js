...
(function () {
  ...
  const DEFAULT_COLS = ['number','subject','priority','status','assigned','due'];
  function getCols()     { try{return JSON.parse(localStorage.getItem('ncm_tcols'))||[...DEFAULT_COLS];}catch{return[...DEFAULT_COLS];} }
  function saveCols(c)   { localStorage.setItem('ncm_tcols',JSON.stringify(c)); }

  // ── Sorting ────────────────────────────────────────────────────────────────
  function _sortTickets(arr) {
  ...
  function renderList(queryOverride) {
    ...
    const headerCells=visibleCols.map(k=>{
      const def=COL_DEFS.find(c=>c.key===k);
      if(!def)return'';
      return _th(k,def.label);
    }).join('');
    ...
    document.getElementById('page-content').innerHTML = `
      <div class="page-body">
        <div class="toolbar">
          ...
          ${NexCRM.Auth.isAdmin()?`<button class="btn btn-ghost" onclick="NexCRM.Tickets.openColumnManager()" title="Manage columns">${Ic('settings',14)}</button>`:''}
        </div>
        <div class="card-flush">
          ${tickets.length
            ? `<table class="data-table"><thead><tr>${headerCells}<th></th></tr></thead><tbody>${rows}</tbody></table>`
    ...
    // Restore search focus + cursor position
        ...
        <div class="detail-grid">
          ...
          <div class="detail-sidebar">
            <div class="card">
              ...
              ${canEdit?`<div class="field-row"><label>Status</label><select class="input-sm" onchange="NexCRM.Tickets.patchField('${t.id}','status',this.value)">${sOpts}</select></div>
  ...
  function _ticketForm(t){
    ...
    const users=NexCRM.Store.Users.getAll().filter(u=>u.active);
    const cO=custs.map(c=>`<option value="${c.id}" ${t&&t.customerId===c.id?'selected':''}>${S.esc(c.name)} — ${S.esc(c.company)}</option>`).join('');
    ...
    return `<div class="form-grid">
      ...
      <div class="form-field"><label>Customer <span class="required">*</span></label><select id="f-customer" class="input"><option value="">Select customer…</option>${cO}</select></div>
  ...
  function openColumnManager(){
    ...
    const body=`<p style="font-size:12px;color:var(--s500);margin-bottom:12px">Select which columns appear in the ticket table.</p>
      <div style="display:flex;flex-direction:column;gap:6px">
        ${COL_DEFS.map(c=>`<div class="col-item">
  ...
  function _toggleCol(key){
    const v=getCols(); const ni=v.indexOf(key);let nc;
    if(ni>=0){if(v.length<=2){NexCRM.toast('At least 2 columns required','error');return;}nc=v.filter(c=>c!==key);}else{nc=[...v,key];}
    saveCols(nc);
    const btn=document.getElementById('ctog-'+key);if(btn)btn.classList.toggle('on',nc.includes(key));
    renderList();
  }
  function _resetCols(){saveCols([...DEFAULT_COLS]);NexCRM.Utils.closeModal();renderList();}

  // ── CSV IMPORT (Manager/Admin only) ───────────────────────────────────────
  function openImportModal(){
    ...
    S.openModal('Import tickets from CSV',
      `<div style="margin-bottom:14px"><p style="font-size:13px;color:var(--s600);line-height:1.6;margin-bottom:10px">Upload a CSV file to import tickets in bulk.</p>
        <div style="background:var(--s50);border-radius:8px;padding:9px 12px;font-size:11px;font-family:monospace;color:var(--s700);overflow-x:auto">subject, description, status, priority, customer_email, assigned_email, due_date</div></div>
  ...
  function downloadTemplate(){const h=['subject','description','status','priority','customer_email','assigned_email','due_date'];const ex=['Login issue after password reset','User unable to login after reset email','new','high','client@example.com','agent@nexcrm.dev','2025-08-15'];const csv=[h.join(','),ex.map(v=>`"${v}"`).join(',')].join('\n');const a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);a.download='nexcrm-import-template.csv';a.click();}
  ...
  function previewCSV(input){const file=input.files[0];if(!file)return;const S=NexCRM.Utils;const reader=new FileReader();reader.onload=e=>{const rows=_parseCSV(e.target.result);_parsedCSV=rows;const preview=document.getElementById('csv-preview');const btn=document.getElementById('csv-import-btn');if(!rows.length){preview.innerHTML=`<p style="color:var(--rose);font-size:13px">No valid rows found.</p>`;btn.disabled=true;return;}btn.disabled=false;preview.innerHTML=`<div style="font-size:12px;color:var(--s600);margin-bottom:8px">${rows.length} ticket${rows.length!==1?'s':''} ready to import</div><div style="max-height:180px;overflow-y:auto;border:1px solid var(--border);border-radius:8px;overflow:hidden"><table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr style="background:var(--s50)">${['Subject','Priority','Status','Customer email'].map(h=>`<th style="padding:7px 10px;text-align:left;color:var(--s500);font-weight:600">${h}</th>`).join('')}</tr></thead><tbody>${rows.slice(0,8).map(r=>`<tr style="border-top:1px solid var(--s100)"><td style="padding:7px 10px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${S.esc(r.subject)}</td><td style="padding:7px 10px">${S.priorityBadge(r.priority||'medium')}</td><td style="padding:7px 10px">${S.statusBadge(r.status||'new')}</td><td style="padding:7px 10px;color:var(--s600)">${S.esc(r.customer_email||'—')}</td></tr>`).join('')}${rows.length>8?`<tr><td colspan="4" style="padding:7px 10px;text-align:center;color:var(--s400)">+${rows.length-8} more…</td></tr>`:''}</tbody></table></div>`;};reader.readAsText(file);}
  function importCSV(){if(!_parsedCSV.length)return;const vSt=['new','assigned','in_progress','pending','resolved','closed'];const vPr=['low','medium','high','critical'];let n=0;for(const row of _parsedCSV){if(!row.subject?.trim())continue;let cId=null;if(row.customer_email){const c=NexCRM.Store.Customers.getAll().find(c=>c.email.toLowerCase()===row.customer_email.toLowerCase());if(c)cId=c.id;}let aId=null;if(row.assigned_email){const u=NexCRM.Store.Users.getByEmail(row.assigned_email);if(u)aId=u.id;}const status=vSt.includes((row.status||'').toLowerCase())?row.status.toLowerCase():'new';const priority=vPr.includes((row.priority||'').toLowerCase())?row.priority.toLowerCase():'medium';NexCRM.Store.Tickets.create({subject:row.subject.trim(),description:row.description||'',status,priority,customerId:cId,assignedToId:aId,dueDate:row.due_date||null});n++;}  _parsedCSV=[];NexCRM.Utils.closeModal();NexCRM.toast(`${n} ticket${n!==1?'s':''} imported`,'success');renderList();}
  ...
  function exportData(){
    ...
    const rows=NexCRM.Store.Tickets.getAll().map(t=>{
      ...
      return{'Ticket Number':t.number,'Subject':t.subject,'Case Description':t.description||'','Status':t.status,'Priority':t.priority,'Customer':S.customerName(t.customerId),'Company':NexCRM.Store.Customers.get(t.customerId)?.company||'','Assigned To':S.userName(t.assignedToId),'Due Date':t.dueDate||'','Created Date':cd?cd.toLocaleDateString():'','Created Time':cd?cd.toLocaleTimeString():'','Updated Date':ud?ud.toLocaleDateString():'','Updated Time':ud?ud.toLocaleTimeString():'','Old Value':'','New Value':'','Edit Date':'','Edit Time':''};
    });
  ...
  window.NexCRM.Tickets = { render,renderList,renderDetail,openCreateModal,openEditModal,create,update,patchField,confirmDelete,addComment,openImportModal,downloadTemplate,previewCSV,importCSV,exportData,openColumnManager,_toggleCol,_resetCols,_setSort,_setQ,_setSt,_setPr,_setCust,_clearFilters };
})();
